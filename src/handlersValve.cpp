...
{
  if (!valve)
    return; // if it's not a valid range
  DEBUG_PRINT("[onValveOpen] id=%d\n", valve->getIdStr);

  // update Homie property
  if (Homie.isConnected())
    valve->getHomie()->setProperty("status").send(boolStr(true));
...
void onValveClose(Valve *valve)
{
  ...
  // update Homie property
  if (Homie.isConnected())
    valve->getHomie()->setProperty("status").send(boolStr(false));
...
bool handleValveStatus(const String &valveId, const String &value)
{
  ...
  if (!valve)
    return false; // if it's not a valid valve
  ...
  if (on)
    valve->open(); // if valve opened manually, then use default run time
  else
    valve->close();
...
bool handleValveRT(const String &valveId, const String &value)
{
  ...
  if (Homie.isConnected())
    valve->getHomie()->setProperty("runtime").send(String(t));
