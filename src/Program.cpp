...
{
  return runDays[weekday(datetime) - 1] && hour(datetime) == startHour && minute(datetime) == startMin;
}
...
void Program::addValve(Valve *valve)
{
  ...
  if (valveCount >= NUMBER_OF_VALVES)
    return;
...
void Program::printConfig()
{
  ...
  CONSOLE(" rt=");
  for (int i = 0; i < valveCount; i++)
    CONSOLE("%d,", getRunTime(i));
  CONSOLE(" rd=");
  for (int i = 0; i < 7; i++)
    CONSOLE("%d,", getRunDay(i));
  CONSOLE("\n");
}
